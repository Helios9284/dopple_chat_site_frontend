...
const PinConfirmModal = ({ pinConfirm }) => {
	const { openPinConfirmModal: open, setOpenPinConfirmModal, pinConfirmContent, dopplesInfo } = useContext(RefContext);
	const [doing, setDoing] = useState(false);
	let container = useRef();
	const _info = dopplesInfo.find(x => x.sender === pinConfirmContent?.dopple_name)

	const close = async () => {
		setOpenPinConfirmModal(false);
	};
				...
				<div className={"fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 outline-none rounded-[10px] bg-nav-desktop w-[calc(100vw-40px)] max-w-[388px]"} ref={container}>
					<div className="flex flex-col items-center p-[30px]">
						...
						<div className="flex justify-center items-center space-x-[5px] font-bold text-[24px] leading-[28px] text-white mt-[30px]">
							...
							<span>Pin / Unpin Chat</span>
						</div>
						<span className="mt-[10px] text-[16px] leading-[19px] text-subtext text-center">Are you sure you want to pin/unpin chat with {_info?.name}?</span>
						<div className="flex items-center space-x-[10px] mt-[30px] w-full">
							...
							<button className="flex justify-center items-center flex-1 h-[45px] rounded-[5px] font-bold text-[16px] leading-[19px] text-white bg-blue2 disabled:opacity-70" onClick={archiveChat} disabled={doing}>Pin / Unpin</button>
						</div>
					</div>
				</div>
			</div>
