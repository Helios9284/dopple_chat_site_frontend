...
const DeleteThreadConfirmModal = ({ deleteConfirm }) => {
	const { openDeleteThreadConfirmModal: open, setOpenDeleteThreadConfirmModal, deleteThreadConfirmContent, dopplesInfo } = useContext(RefContext);
	const [doing, setDoing] = useState(false);
	let container = useRef();
	const _info = dopplesInfo.find(x => x.sender === deleteThreadConfirmContent?.dopple_name)

	const close = async () => {
		setOpenDeleteThreadConfirmModal(false);
	};
					...
					<Zoom in timeout={300}>
						<div className="bg-nav-desktop">
							<div className="flex flex-col items-center p-[30px]">
								...
								<span className="mt-[10px] text-[16px] leading-[19px] text-subtext text-center">Are you sure you want to delete this message?</span>
...
export default DeleteThreadConfirmModal;
