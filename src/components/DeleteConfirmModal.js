...
const DeleteConfirmModal = ({ deleteConfirm }) => {
	const { openDeleteConfirmModal: open, setOpenDeleteConfirmModal, deleteConfirmContent, dopplesInfo } = useContext(RefContext);
	const [doing, setDoing] = useState(false);
	let container = useRef();
	const _info = dopplesInfo.find(x => x.sender === deleteConfirmContent?.dopple_name)
			...
			<div className="absolute w-full h-full bg-inputback backdrop-blur-[5px]">
				<div className={"fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 outline-none rounded-[10px] bg-nav-desktop w-[calc(100vw-40px)] max-w-[388px]"} ref={container}>
					<div className="flex flex-col items-center p-[30px]">
						...
						<span className="mt-[10px] text-[16px] leading-[19px] text-subtext text-center">Are you sure you want to delete your chat with {_info?.name}?</span>
